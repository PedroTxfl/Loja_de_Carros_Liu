...
import { ClienteService, ClienteData, UpdateClienteData } from '../services/Clienteservice';
...
clienteRouter.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errorMessage = err.msg || 'Erro interno do servidor ao inserir cliente.';
...
// GET /clientes/:id
clienteRouter.get('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
...
// PUT /clientes/:id
clienteRouter.put('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id);
    if (isNaN(id)) {
      res.status(400).json({ error: 'ID do cliente deve ser um número.' });
...
// DELETE /clientes/:id
clienteRouter.delete('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const id = parseInt(req.params.id);
    if (isNaN(id)) {
      res.status(400).json({ error: 'ID do cliente deve ser um número.' });
